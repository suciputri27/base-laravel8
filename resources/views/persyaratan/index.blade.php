    ...
    <script>
        function PersyaratanRow(item) {
            ...
            return '<tr>' +
                ...
                '<td>' + status + '</td>' +
                '<td>' +
                    '<button type="button" class="btn btn-secondary btn-sm" onclick="editPersyaratan(\'' + item.encrypted_id + '\', this)" data-name="' + App.escapeHtml(item.nama_persyaratan) + '" data-description="'  + '" data-active="' + (item.is_active ? '1' : '0') + '"><i class="fas fa-edit"></i></button> ' +
        ...
        function editPersyaratan(encryptedId, button) {
            ...
            document.getElementById('PersyaratanForm').action = '{{ url('persyaratan') }}/' + encryptedId;
        ...
        function deletePersyaratan(encryptedId) {
            App.confirm({
                ...
                App.submitData({
                    url: '{{ url('persyaratan') }}/' + encryptedId,
                    method: 'DELETE',
